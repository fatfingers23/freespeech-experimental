...
function TilePad(props) {
    ...
    function updatePage(page) {
        if(futureHistory().length > 0) // Clear the future history if there is any.
            setFutureHistory([]);
    ...
    return (
        ...
        <div class={styles.tilepad}>
            <For each={props.tileData()[page()]} fallback={<div>Loading...</div>}>
                {(item) => <Tile {...item} callback={updatePage} />}
            </For>
        </div>
        </>
    );
